    ...
    void (async () => {
      ...
      transport
          ...
          whenShellSettled(() => {
            try {
              const cancelDelivery = deliverCommand(
                ...
                cmd,
                (submitted) => onSettled?.(submitted && !life.dead)
              )
        ...
        if (data.initialCommand) {
          ...
          const st = useAgentStatus.getState().byId[id]
          // Two sources of a session id, and they mean different things:
          //  - `hookSessionId` (st.sessionId) is a HOOK-CONFIRMED id — a SessionStart fired, so a
          //    transcript exists on disk and `--resume <id>` is safe.
          //  - `mintedSessionId` (data.agentSessionId) is the id we MINTED at creation and launched
          //    with `--session-id`. It is known BEFORE any turn; resuming it would error when no
          //    transcript was ever written (the "switched model/subscription before any turn" case).
          // So resume ONLY off the hook-confirmed id; when only the minted id is known, re-launch
          // FRESH under that id (the same --session-id flag first launch used) instead of --resume.
          const hookSessionId = st?.sessionId
          const mintedSessionId = data.agentSessionId
          ...
          //
          // Inheritance-aware: both assemblers resolve a custom agent's baseAgent/args, so a
          // claude-base proxy resumes/re-launches with its own binary + claude's grammar + its
          // custom args — the SAME builder fresh launch uses, so cold restore and first launch
          // can't drift. For a builtin this is byte-identical to the old resumeCommand +
          // withPermissionMode path.
          //
          ...
          if (hookSessionId) {
            modelRespawnTrace('cold-resume.assemble', {
              nodeId: id,
              agentId,
              mode: 'resume',
              model: data.agentModel,
              hasLaunchOverride: !!launchCmdOverride
            })
            cmd = assembleResumeCommand(
              {
                agentId,
                customAgent,
            ...
            modelRespawnTrace('cold-resume.assemble', {
              nodeId: id,
              agentId,
              mode: 'fresh-with-minted-id',
              model: data.agentModel,
              sessionIdFlagSupported,
              hasLaunchOverride: !!launchCmdOverride
            })
            cmd = assembleLaunchCommand(
              {
                agentId,
                customAgent,
                permissionMode: mode,
                sessionId: sessionIdFlagSupported ? mintedSessionId : undefined,
                sessionIdFlagSupported,
                sharedIdentity: shared,
                model: data.agentModel,
                launchCmdOverride
          ...
          modelRespawnTrace('cold-resume.ready', {
            nodeId: id,
            agentId,
            hasHookSessionId: !!hookSessionId,
            hasMintedSessionId: !!mintedSessionId,
            commandBuilt: !!cmd
          })
          if (cmd) {
            writeWhenShellReady(cmd, (live) => {
              if (!live) {
                reportThisRespawn({
                  ok: false,
                  detail: 'the replacement terminal ended during resume delivery'
                })
                return
              }
              ...
              void waitForAgentRespawnProcess(() => api.pty.agentProcess(id, agentId), {
                active: () => !life.dead && agentRespawnPending(id, respawnGeneration),
